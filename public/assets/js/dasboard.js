...
async function loadTema(clusterId, year, startDate, endDate) {
    try {
        ...
        const response = await axios.get(`${baseUrl}/get-tema`, {
            withCredentials: true,
            params,
        });
...
async function loadPermasalahan(temaId, year, startDate, endDate) {
    try {
        ...
        const response = await axios.get(
            `${baseUrl}/get-permasalahan-by-tema`,
        ...
        permasalahans.forEach((item) => {
            ...
            option.textContent =
                item.unique_namespace + " " + item.permasalahan;
            selectElement.appendChild(option);
        });
    } catch (error) {
...
async function loadRenaksi(permasalahanId, year, startDate, endDate) {
    try {
        ...
        const response = await axios.get(
            `${baseUrl}/get-rencana-aksi-permasalahan`,
            { withCredentials: true, params }
        );
        ...
        renaksi.forEach((item) => {
            ...
            option.value = item.id;
            option.textContent =
                item.unique_namespace + " " + item.rencana_aksi;
            selectElement.appendChild(option);
        });
    } catch (error) {
...
document
    ...
    .addEventListener("change", function () {
        ...
        var themeChartContainer = document.getElementById(
            "theme-chart-container"
        );
        var themeChartContainer2 = document.getElementById(
            "theme-chart-container2"
        );
...
document
    ...
    .addEventListener("change", async function () {
        ...
        if (idTema) {
            ...
            const fetchData = await rataRata(
                idTema,
                selectedYear,
                startDate,
                endDate
            );
            const fetchDataAnggaran = await rataRataAnggaran(
                idTema,
                selectedYear,
                startDate,
                endDate
            ...
            const dataValues = fetchData.map((item) =>
                parseFloat(item.rata_rata_capaian).toFixed(1)
            );

            const labelsAnggaran = fetchDataAnggaran.map(
                (item) => item.permasalahan
            );
            const dataValuesAnggaran = fetchDataAnggaran.map((item) =>
                parseFloat(item.rata_rata_capaian).toFixed(1)
            );
...
document
    ...
    .addEventListener("change", async function () {
        ...
        var renaksiChartContainer = document.getElementById(
            "renaksi-chart-container"
        );
        ...
        if (idPermasalahan) {
            ...
            try {
                const fetchDataRenaksiPenyelesaian =
                    await capaianRenaksiPenyelesaian(
                        idPermasalahan,
                        selectedYear,
                        startDate,
                        endDate
                    );
                const fetchDataRenaksiAnggaran = await capaianRenaksiAnggaran(
                    idPermasalahan,
                    selectedYear,
                    startDate,
                    endDate
                ...
                const labels = fetchDataRenaksiPenyelesaian.map(
                    (item) => item.renaksi
                );
                const dataValues = fetchDataRenaksiPenyelesaian.map((item) =>
                    parseFloat(item.capaian).toFixed(1)
                );

                const labelsAnggaran = fetchDataRenaksiAnggaran.map(
                    (item) => item.renaksi
                );
                const dataValuesAnggaran = fetchDataRenaksiAnggaran.map(
                    (item) => parseFloat(item.capaian).toFixed(1)
                );
...
document
    ...
    .addEventListener("change", async function () {
        ...
        if (idRenaksi) {
            ...
            const fetchDataTWPenyelesaian = await TWPenyelesaian(
                idRenaksi,
                selectedYear,
                startDate,
                endDate
            ...
            const dataTarget = fetchDataTWPenyelesaian.map(
                (item) => item.target
            );
            const dataCapaian = fetchDataTWPenyelesaian.map(
                (item) => item.capaian
            );
...
async function rataRata(id, year, startDate, endDate) {
    ...
    const response = await axios.get(`${baseUrl}/get-capaian-permasalahan`, {
        withCredentials: true,
        params,
    });
...
async function rataRataAnggaran(id, year, startDate, endDate) {
    ...
    const response = await axios.get(`${baseUrl}/get-capaian-anggaran`, {
        withCredentials: true,
        params,
    });
    return response.data.data;
...
async function capaianRenaksiPenyelesaian(id, year, startDate, endDate) {
    ...
    const response = await axios.get(
        `${baseUrl}/get-capaian-renaksi-penyelesaian`,
        { withCredentials: true, params }
    );
    return response.data.data;
}
...
async function capaianRenaksiAnggaran(id, year, startDate, endDate) {
    ...
    const response = await axios.get(
        `${baseUrl}/get-capaian-renaksi-anggaran`,
        { withCredentials: true, params }
    );
    return response.data.data;
}
...
async function TWPenyelesaian(id, year, startDate, endDate) {
    ...
    const response = await axios.get(`${baseUrl}/get-capaian-tw-penyelesaian`, {
        withCredentials: true,
        params,
    });
    return response.data.data;
...
async function fetchData(year, startDate, endDate) {
    try {
        ...
        document.getElementById("pemasalahan-count").innerText =
            counts.permasalahan || 0;
        document.getElementById("rencana-aksi-count").innerText =
            counts.renaksi || 0;
        document.getElementById("penyelesaian-count").innerText =
            counts.penyelesaian || 0;
        document.getElementById("anggaran-count").innerText =
            counts.anggaran || 0;
    } catch (error) {
...
async function updateAllCharts() {
    ...
    if (idTema) {
        ...
        if (window.myBarChart && window.myBarChart2) {
            window.myBarChart.data.labels = fetchData.map(
                (item) => item.permasalahan
            );
            window.myBarChart.data.datasets[0].data = fetchData.map((item) =>
                parseFloat(item.rata_rata_capaian).toFixed(1)
            );
            window.myBarChart.update();

            window.myBarChart2.data.labels = fetchDataAnggaran.map(
                (item) => item.permasalahan
            );
            window.myBarChart2.data.datasets[0].data = fetchDataAnggaran.map(
                (item) => parseFloat(item.rata_rata_capaian).toFixed(1)
            );
            window.myBarChart2.update();
        }
    }
    ...
    if (idPermasalahan) {
        const fetchDataRenaksiPenyelesaian = await capaianRenaksiPenyelesaian(
            idPermasalahan,
            selectedYear,
            startDate,
            endDate
        ...
        if (window.myProblemChart && window.myProblemChart2) {
            window.myProblemChart.data.labels =
                fetchDataRenaksiPenyelesaian.map((item) => item.renaksi);
            window.myProblemChart.data.datasets[0].data =
                fetchDataRenaksiPenyelesaian.map((item) =>
                    parseFloat(item.capaian).toFixed(1)
                );
            window.myProblemChart.update();

            window.myProblemChart2.data.labels = fetchDataRenaksiAnggaran.map(
                (item) => item.renaksi
            );
            window.myProblemChart2.data.datasets[0].data =
                fetchDataRenaksiAnggaran.map((item) =>
                    parseFloat(item.capaian).toFixed(1)
                );
            window.myProblemChart2.update();
        }
    ...
    if (idRenaksi) {
        ...
        if (window.myRenaksiChart) {
            window.myRenaksiChart.data.labels = fetchDataTWPenyelesaian.map(
                (item) => item.triwulan
            );
            window.myRenaksiChart.data.datasets[0].data =
                fetchDataTWPenyelesaian.map((item) => item.target);
            window.myRenaksiChart.data.datasets[1].data =
                fetchDataTWPenyelesaian.map((item) => item.capaian);
            window.myRenaksiChart.update();
        }
    }
}
